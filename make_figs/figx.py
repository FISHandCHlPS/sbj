    ...
    for i in range(100):
        ...
        if dfui.intervenue == -1:
            ...
            if dfui.intervenue_type == 0:
                print("error", dfui.intervenue_type, dfui.uid, dfui.sid, dfui.intervenue)
                ng = True
    if len(data) >= 2:
        x, y = zip(*data)
...
g = sns.jointplot(dfx, x="coef1", y="thr1", xlim=(0, 5.8), ylim=(-2, 12), hue="type", palette=["gray", "red", "blue"], marginal_kws=dict(weights=dfx.weight))
g.plot_joint(sns.kdeplot, levels=3)
plt.savefig("figs/figx.pdf")
